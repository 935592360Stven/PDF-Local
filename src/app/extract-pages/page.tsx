...
import { Loader2, Download, Scan } from "lucide-react"
...
export default function ExtractPagesPage() {
  ...
  const go = async () => {
    ...
    setProcessing(true); setProgress("Extracting pages…")
    try {
      const { PDFDocument } = await import("pdf-lib")
      const doc = await PDFDocument.load(await files[0].arrayBuffer())
      const indices = Array.from(selected).map(n => n - 1).sort((a, b) => a - b)
      const out = await PDFDocument.create()
      const cp = await out.copyPages(doc, indices)
  ...
  const dl = () => { if (!blob) return; const u = URL.createObjectURL(blob); const a = document.createElement("a"); a.href = u; a.download = `extracted-${files[0]?.name || "output.pdf"}`; a.click(); URL.revokeObjectURL(u) }

  return (
    <ToolLayout title="Extract Pages" description="Select specific pages and extract them into a new PDF." icon={<Scan className="h-7 w-7 text-[#34c759]" />}>
        ...
        <div className="mt-8">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-3">
              ...
              <span className="text-[12px] text-[#86868b]">{selected.size} page{selected.size > 1 ? "s" : ""} selected</span>
            </div>
          </div>
            ...
            {Array.from({ length: displayPages }, (_, i) => i + 1).map(n => {
              ...
              return (
                <button key={n} onClick={() => togglePage(n)}
                  ...
                  {isSelected && (
                    <div className="absolute inset-0 flex items-center justify-center bg-[#34c759]/15">
                      <div className="flex h-7 w-7 items-center justify-center rounded-full bg-[#34c759] text-[11px] font-bold text-white shadow-md">✓</div>
                    </div>
                  )}
                  <div className={cn("absolute bottom-1.5 left-1.5 rounded-md px-1.5 py-0.5 text-[10px] font-medium",
                    isSelected ? "bg-[#34c759] text-white" : "bg-black/50 text-white")}>
                    {n}
                  </div>
      ...
      <div className="mt-8 flex flex-col items-center gap-4">
        <button onClick={go} disabled={!files.length || !selected.size || processing || rendering}
          className={cn("inline-flex items-center gap-2 rounded-[12px] px-7 py-2.5 text-[15px] font-medium text-white transition-all active:scale-[0.97]",
            !files.length || !selected.size || processing || rendering ? "bg-[#e8e8ed] text-[#c7c7cc] cursor-not-allowed" : "bg-[#34c759] hover:bg-[#2db84d]")}>
          {processing && <Loader2 className="h-4 w-4 animate-spin" />}
          {processing ? progress || "Extracting…" : !files.length ? "Upload a PDF file" : !selected.size ? "Select pages to extract" : `Extract ${selected.size} page${selected.size > 1 ? "s" : ""}`}
        </button>
        {blob && (
          <div className="flex flex-col items-center gap-8">
            <div className="inline-flex items-center gap-2.5 rounded-full bg-[#F2F2F7] px-5 py-2 text-[14px] text-[#6e6e73]">
              <span className="text-[#34c759]">✓</span>
              <span>{selected.size} page{(selected.size) !== 1 ? "s" : ""} extracted — {(blob.size / 1024).toFixed(0)} KB</span>
            </div>
            <button onClick={dl} className="inline-flex items-center gap-2 rounded-[12px] bg-[#34c759] px-7 py-2.5 text-[15px] font-medium text-white transition-all hover:bg-[#2db84d] active:scale-[0.97]">
      ...
      <div className="mt-12 pt-8">
        <h3 className="text-[14px] font-semibold text-[#1d1d1f]">How to extract pages</h3>
        <ol className="mt-3 space-y-2 text-[14px] text-[#86868b] list-decimal list-inside">
          ...
          <li>Click the pages you want to extract (they turn green with ✓)</li>
          <li>Click &ldquo;Extract&rdquo; and download the new PDF</li>
        </ol>
      </div>
    </ToolLayout>
  )
